...
const regex = /(?<!\{)\{\{([^{}]+)\}\}(?!\})/g;
const extractVariablesFromTemplate = (plainContent: string) => {
...
export const genOne = async ({
  ...
  return generateTypeDefinition({
    ...
    interfaceName: interfaceName
      // kebab-case to UpperCamel
      ? interfaceName.charAt(0).toUpperCase() +
        interfaceName.slice(1).replace(/-./g, (x) => x[1].toUpperCase())
      : "AutoGenerated",
  });
};
...
export const genAll = async ({
  ...
  const types = await Promise.all(
    templates.map(async ({ id, name }) => ({
      def: await genOne({ templateId: id, interfaceName: name }),
      name,
      id,
    }))
  );
